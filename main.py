...
def bbox_detection(prediction, conf_threshold, n_classes, nms_conf = 0.4):
    ...
    for ind in range(batch_size):  
        ...
        max_conf, max_conf_score = torch.max(img_pred[:,5:5+ n_classes], 1)
        max_conf = max_conf.float().unsqueeze(1)
        max_conf_score = max_conf_score.float().unsqueeze(1)

        # concat probabilities with box attributes
        combined = (img_pred[:,:5], max_conf, max_conf_score)
        img_pred = torch.cat(combined, 1) 

        # class index will come from those who have objectness > object confidence threshold
        potential_idx =  (torch.nonzero(img_pred[:,4])) 
        img_pred_ = img_pred[potential_idx.squeeze(),:].view(-1,7) # 5(box_attribute + objectness)+2(prob)
...
if __name__ == '__main__':
    ...
    scaling_factor = torch.min(conf_inp_h/im_dim_list,1)[0].view(-1,1)
    output[:,[1,3]] -= (conf_inp_h - scaling_factor*im_dim_list[:,0].view(-1,1))/2
    output[:,[2,4]] -= (conf_inp_h - scaling_factor*im_dim_list[:,1].view(-1,1))/2
    output[:,1:5] /= scaling_factor
    # print(output)

    output[:,1:5] = torch.clamp(output[:,1:5], 0.0, float(conf_inp_h))
